...
//
//public class ObstacleEditorUtil {
//
...
//        // We cannot assume anything about the ordering of the control points
//        // handed to us, but we must be able to
//        // access them by location index and altitude index. To achieve this we
//        // place them in a map that will be
...
//    }
//
//    public static final Position createPositionFor(final WorldWindow wwd, final Line ray,
//            final double referenceElevation, final EdgeInfo edge) {
//        // If the nearest point occurs before the line segment, then insert the
//        // new point before the segment. If the
//        // nearest point occurs after the line segment, then insert the new
//        // point after the segment. If the nearest
//        // point occurs inside the line segment, then insert the new point in
//        // the segment.
//
//        Vec4 newPoint = intersectGlobeAt(wwd, referenceElevation, ray);
...
//    }
//
//    public static List<EdgeInfo> computeEdgeInfoFor(final List<? extends LatLon> locations) {
//        // Compute edge data structures for the segment between each successive
//        // pair of control points, including the
//        // edge between the last and first control points. Do this for the upper
//        // and lower altitudes of the airspace.
//        // We cannot assume anything about the ordering of the control points
//        // handed to us, but we must be able to
//        // access them by location index and altitude index. To achieve this we
//        // place them in a map that will be
//        // indexable by location and altitude.
//
//        final List<EdgeInfo> edgeInfoList = new ArrayList<EdgeInfo>();
//
//        final HashMap<Object, AirspaceControlPoint> map = new HashMap<Object, AirspaceControlPoint>();
//        for (AirspaceControlPoint p : controlPoints) {
//            map.put(p.getKey(), p);
//        }
//
...
//                edgeInfoList.add(new EdgeInfo(locationIndex, nextLocationIndex, altitudeIndex, controlPoint.getPoint(),
//                        nextControlPoint.getPoint()));
//            }
//        }
//
//        return edgeInfoList;
//    }
//
//    public static EdgeInfo selectBestEdgeMatch(final WorldWindow wwd, final Line ray, final double referenceElevation,
//            final List<? extends EdgeInfo> edgeInfoList) {
//        // Try to find the edge that is closest to the given ray. This is used
//        // by the routine doAddNextLocation(),
//        // which is trying to determine the user's intent as to which edge a new
//        // two control points should be added to.
//        // Therefore consider the potential locations of a new control point on
//        // the ray: one for each of the lower
//        // and upper airspace altitudes. We choose the edge that is closest to
//        // one of these points. We will ignore
//        // an edge if its nearest point is behind the ray origin.
//        final Vec4 pointOnLine = intersectGlobeAt(wwd, referenceElevation, ray);
//
